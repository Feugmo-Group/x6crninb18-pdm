...
"""Paper figures F8-F10 (PAPER_PLAN.md Section 5, the spatial model items) under
outputs/paper/.
  ...
  fig8_steady_spatial.png  — (a) D-cancellation: nondim steady profile shape
    ...
    species, from the steady spatial solve checkpoint (outputs/spatial/steady_{OV,CV}.npz).
  fig9_transient_qs.png — quasi-steady lag: transient NSEM profile vs the
    instantaneous quasi-steady shape at tau=1, both species, from the the transient spatial solve
    checkpoint (outputs/spatial/transient_{OV,CV}.npz), with the R1 lag
    annotated.
  fig10_composition_edx.png — copied from outputs/plot_composition_map.png
    (src/composition_map.py --fitted), already publication-quality.

Run:  python scripts/make_paper_figs_spatial.py   (AFTER
      src/spatial_steady_trainer.py, src/spatial_transient_trainer.py, and
      src/composition_map.py --fitted have been run)
"""
...
import htw_pdm.spatial_physics as spatial_physics  # noqa: E402
...
SPATIAL_DIR = ROOT / "outputs" / "spatial"
...
# real spatial_physics code path, not a re-derivation) at three D values spanning
...
D_OV_0, D_CV_0 = spatial_physics.D_OV_CM2_S, spatial_physics.D_CV_CM2_S
D_scales = [1e-2, 1.0, 1e2]
profiles = {"OV": [], "CV": []}
c0_by_scale = {"OV": [], "CV": []}
Pe_by_scale = {"OV": [], "CV": []}
for scale in D_scales:
    spatial_physics.D_OV_CM2_S = D_OV_0 * scale
    spatial_physics.D_CV_CM2_S = D_CV_0 * scale
...
spatial_physics.D_OV_CM2_S, spatial_physics.D_CV_CM2_S = D_OV_0, D_CV_0  # restore

g0 = species_groups(p1, 134.0)  # ~480 h the zero-dimensional fit barrier thickness, default D
max_shape_dev = {}
...
rows = []
for name, c in (("OV", "tab:blue"), ("CV", "tab:red")):
    f = SPATIAL_DIR / f"steady_{name}.npz"
    if not f.exists():
        ax_b.text(0.5, 0.5, f"MISSING: {f.name}\n(run src/spatial_steady_trainer.py)",
...
for k, (ax, name) in enumerate(zip(axes9, ("OV", "CV"))):
    f = SPATIAL_DIR / f"transient_{name}.npz"
    if not f.exists():
        ax.text(0.5, 0.5, f"MISSING: {f.name}\n(run src/spatial_transient_trainer.py)",
...
src10 = ROOT / "outputs" / "plot_composition_map.png"
if src10.exists():
    shutil.copy(src10, OUT / "fig10_composition_edx.png")
    print(f"Copied: {src10} -> outputs/paper/fig10_composition_edx.png")
else:
    print(f"MISSING: {src10} (run: python -m htw_pdm.composition_map --fitted)")
