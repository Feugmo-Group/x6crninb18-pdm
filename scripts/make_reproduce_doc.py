...
Module scope is the operative part -- htw_pdm.physics and htw_pdm.spatial_physics
...
# One artefact is produced by a classical script but read out of training
# checkpoints, so the command's own tier understates what it needs. The script
# says so at run time; the table has to as well.  The F-1 exhibit used to be in
# the same position and no longer is: it is built from f1_runs.json.
ARTEFACT_ENV = {
...
PRIMARY = {
    ...
    "paper/field_closure.json": "python -m htw_pdm.field_closure_solve",
    "plot_uncertainty_ensemble.png": "python -m htw_pdm.uncertainty_ensemble",
    "plot_composition_map.png": "python -m htw_pdm.composition_map",
    "spatial_identifiability.csv": "python -m htw_pdm.spatial_identifiability",
    "spatial_inverse_fit.csv": "python -m htw_pdm.spatial_inverse",
    "spatial_inverse_fit.json": "python -m htw_pdm.spatial_inverse",
    "nickel_identifiability.csv": "python -m htw_pdm.nickel_identifiability",
    "nickel_identifiability_verdict.json": "python -m htw_pdm.nickel_identifiability",
...
# Manuscript float -> artefact.  The float numbers are read out of the built
# .aux files when the manuscript sources are present locally, so this table
# cannot drift from the paper as figures are promoted or demoted.  The mapping
# from a label to the artefact it draws is the stable part and lives here; the
# numbering is not.  Without the sources, the artefacts are listed unnumbered.
LABEL_TO_ART = {
    "fig:schematic":      "paper/fig1_model_schematic.png",
    "fig:nsem_workflow":  "paper/fig_nsem_workflow.png",
    "fig:fits":           "paper/fig3_fits_vs_data.png",
    "fig:profile":        "paper/fig4_profile_likelihood.png",
    "fig:ensemble":       "paper/fig5_uncertainty_ensemble.png",
    "fig:longtime":       "paper/fig6_long_time.png",
    "fig:sensitivity":    "paper/fig7_envelope_lines.png",
    "fig:composition":    "paper/fig10_composition_edx.png",
    "fig:f1":             "paper/fig11_f1_failure.png",
    "fig:closure":        "paper/fig12_field_closure.png",
    "fig:baseline":       "paper/fig2_baseline_verification.png",
    "fig:transient":      "paper/fig9_transient_qs.png",
    "fig:maps":           "paper/figS3_sensitivity_maps.png",
    "fig:steady":         "paper/fig8_steady_spatial.png",
    "tab:ladder":         "paper/table2_model_ladder.csv",
    "tab:params":         "paper/table1_parameters.csv",
    "tab:envelope":       "paper/table8_envelope_factors.csv",
    "tab:pbrclosure":     "paper/table10_pbr_closure.csv",
    "tab:f1":             "paper/table4_f1_exhibit.csv",
    "tab:t2a":            "paper/table_spatial_identifiability.csv",
    "tab:ni-fit":         "paper/table_nickel_zone_fit.csv",
    "tab:ni-closures":    "paper/table_nickel_closure_gate.csv",
}


def _float_numbers():
    """label -> printed number, from the built manuscript, if it is here."""
    out = {}
    for aux in (ROOT / "paper" / "main-snjnl.aux",
                ROOT / "paper" / "supplementary.aux"):
        if not aux.exists():
            continue
...
_num = _float_numbers()
FIG = {}
for _lab, _art in LABEL_TO_ART.items():
    _kind = "Fig." if _lab.startswith("fig:") else "Table"
    _n = _num.get(_lab)
    FIG[f"{_kind} {_n}" if _n else f"{_kind} ({_lab.split(':')[1]})"] = _art
FIG = dict(sorted(FIG.items(), key=lambda kv: (
    kv[0].startswith("Table"), kv[0].split()[1].startswith("("),
    kv[0].split()[1].startswith("S"),
    int(kv[0].split()[1].lstrip("S").rstrip(")")) if kv[0].split()[1].lstrip("S").rstrip(")").isdigit() else 0)))
...
for a in unowned:
    ...
    c = ("`python -m htw_pdm.trainer`" if top.startswith("forward")
         ...
         else "`python -m htw_pdm.spatial_identifiability`" if top == "spatial"
         else "—")
