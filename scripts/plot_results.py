...
# Single shared legend for the four repeated-entry panels, in panel (a).
# Legend goes in the Fe linear panel (c): its lower-right corner is empty,
# whereas in (a) the box sits over the three fitted curves.
axes[1][0].legend(fontsize=6.0 * PT, loc="lower right", framealpha=0.95)
...
for xi, ri in zip(x_pos, res_r):
    ax_res.text(xi, ri + (0.12 if ri >= 0 else -0.12), f"{ri:+.2f}",
                ha="center", va="bottom" if ri >= 0 else "top", fontsize=6.5 * PT)
...
ax_ni.errorbar(t_ni, L_ni, yerr=sd_ni, fmt="ks", ms=6, capsize=3,
               label="Veile 2024 data")
ax_ni.axhline(L_ni.mean(), color="grey", ls=":", lw=1.5,
              label=f"mean = {L_ni.mean():.1f} nm (not a fitted PDM layer)")
ax_ni.set_xlabel("t (h)")
ax_ni.set_ylabel("L (nm)")
ax_ni.set_title("Ni — enrichment zone (interfacial exclusion, not modeled)",
                fontsize=8 * PT)
ax_ni.legend(fontsize=6.5 * PT)
