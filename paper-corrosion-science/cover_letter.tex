...
is that several of them are not determined---with consequences large
enough to change a lifetime assessment.
...
carried by three chromium points---80\% by a single measurement. These
...
\item \textbf{The consequences are quantitative and actionable.} The
...
exposure beyond roughly 3000\,hours would discriminate the two---a
concrete experiment this analysis motivates.
...
field strength does not survive a self-consistency test. I would rather
state these bounds than have them inferred.

\item \textbf{A diagnosed and remedied failure mode.} I document an
inversion failure specific to sparse experimental data---the data loss
satisfied while the governing equation is not---and remedy it with a
