"""The steady spatial solve — steady 1-D spatial solve: Newton BVP reference + NSEM vs Newton.

Solves the steady defect-transport BVP (src/spatial_physics.py) for both species
(OV, CV) at the 480 h the zero-dimensional fit geometry, three ways:
...
Acceptance (the spatial design notes milestone the steady spatial solve): NSEM rel Linf vs Newton <= 0.5 % per
species; Newton vs analytic <= 1e-10.

Run:
    python -m htw_pdm.spatial_steady_trainer
    python -m htw_pdm.spatial_steady_trainer domain.Nx=32 train.n_adam=4000
"""
...
def main(cfg: DictConfig) -> dict:
    ...
    for name, g in groups.items():
        ...
        # IC-style pretrain to the Robin plateau (flat start, the zero-dimensional fit discipline).
    ...
    logger.info(f"the steady spatial solve acceptance: Newton-vs-analytic {worst_newton:.2e} (<=1e-10), "
