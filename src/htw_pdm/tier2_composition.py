"""T2-D — composition-vs-depth map + EDX convolution, forward vs Veile features.

Assembles element-fraction depth profiles at the three Veile exposures from the
Tier-1/Tier-2 forward chain — NO fitting to the line scans:

  geometry     L_bl(t) from the Tier-1 closed form (M4 kinetics)
  ...
  Ni zone      exclusion closure with nominal (pre-T2-E) parameters r_Ni, D_Ni_eff:
               w_Ni(t) = D_Ni_eff / v_m(t), v_m = (dL_bl/dt)/PBR (recession speed),
               excess(t) = Ni_matrix*(1-r_Ni)*d_consumed(t), A_Ni = Ni_m + excess/w
               Pass --fitted after running tier2_inverse.py to use the T2-E values.
  gradient     g_bl predicted from the T2-B cation-vacancy profile: site-fraction
               modulation = c0_CV * (chat(1)-chat(0)) / N_cation(spinel) — compared
               against the T2-A minimum-detectable gradient (the no-go check).

The profiles reuse the T2-A shape builder (tier2_identifiability.profiles) with the
...
# Nominal (pre-T2-E) Ni-exclusion parameters; overwritten by --fitted.
NI_NOMINAL = {"r_Ni": 0.0, "D_Ni_eff_nm2_h": 2.3, "phi_ol_supply": 0.0}

EXPOSURES_H = [72.0, 168.0, 480.0]


def tier1_geometry(p1: Parameters, t_h: float):
...
def ni_zone(p1: Parameters, t_h: float, ni: dict, fitted: bool = False):
    ...
    fitted=True: solve the same Wagner moving-frame PDE the T2-E parameters were
    ...
    """
    if fitted:
        from htw_pdm.tier2_inverse import wagner_solve
...
def predicted_gradient(p1: Parameters, t_h: float) -> float:
    """g_bl (abs %) from the T2-B cation-vacancy profile at exposure t."""
    L, _ = tier1_geometry(p1, t_h)
...
def main(fitted: bool = False):
    ...
    ni = dict(NI_NOMINAL)
    tag = "nominal"
    fit_file = OUT / "tier2_inverse_fit.json"
    ...
    for ipanel, (ax, t_h) in enumerate(zip(axes, EXPOSURES_H)):
        L, _ = tier1_geometry(p1, t_h)
        w_ni, a_ni = ni_zone(p1, t_h, ni, fitted=(tag == "T2-E fitted"))
    ...
    out_png = OUT / "plot_tier2_composition.png"
    plt.savefig(out_png, dpi=300)
    print(f"Saved: {out_png}")

    with open(OUT / "tier2_composition_comparison.csv", "w", newline="") as fcsv:
        w = csv.writer(fcsv)
        w.writerow(["t_h", "feature", "predicted", "measured_mean", "measured_sd",
                    "rel_dev"])
        w.writerows(rows)
    print(f"Saved: {OUT / 'tier2_composition_comparison.csv'}")
    for r in rows:
        print("  ", r)
