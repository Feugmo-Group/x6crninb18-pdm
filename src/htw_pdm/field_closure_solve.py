"""The field-closure solve runner — is the PDM's constant-field closure self-consistent?

Four questions, in order:

  A1 VERIFICATION. With the space charge switched off (lam_scale = 0) the coupled
     solver must reproduce the the spatial model analytic constant-field solution exactly.
  ...
  A2 MAGNITUDE. Evaluate the space-charge parameter Lam and the Debye length at
     The identified the zero-dimensional fit kinetics and the the spatial model assumed transport constants.
  ...
  A4 NEURAL SOLVE. Grade an NSEM solve of the coupled nonlinear system against
     The Newton reference, inside the convergent regime. Unlike the the spatial model steady
...
Writes outputs/paper/field_closure.json.
...
from htw_pdm.field_closure import (  # noqa: E402
    E_CHARGE_C,
    EPS_0_F_CM,
    EPS_R,
    EPS_R_SCAN,
...
def a1_verification(species, D1, w, xi_np) -> dict:
    """lam_scale = 0 must reproduce the the spatial model analytic solution exactly."""
...
def a4_nsem(species, D1, w, xi, lam_scale: float, seed: int = 0) -> dict:
    ...
    # to learn the space-charge correction (same discipline as the transient spatial solve pretraining).
    tgt_c = torch.stack(
...
def main() -> dict:
    ...
    print(f"[the field-closure solve] L({T_EVAL_H:.0f} h) = {L_nm:.3f} nm, eps_r = {EPS_R}, "
          f"{N_NODES} nodes")
    for sp in species:
    ...
    path = os.path.join(out_dir, "field_closure.json")
    with open(path, "w") as fh:
        json.dump(res, fh, indent=2)
    print(f"[the field-closure solve] wrote {path}")
    return res
