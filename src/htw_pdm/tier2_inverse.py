"""T2-E — Tier-2 inverse restricted to the T2-A-identifiable parameter set.

Fits the Ni-exclusion closure to the REAL Veile Ni observables. Per the T2-A gate,
...
metal ahead of the interface):
    ...
    v_m(t) = (dL_bl/dt)/PBR_bl + phi_ol*(dL_ol/dt)/PBR_ol   (Tier-1 kinetics)
...
physical heterogeneity (same reasoning as the T2-A gate).
...
Run:  python -m htw_pdm.tier2_inverse   (~3-5 min)
"""
...
def main():
    ...
    fig.suptitle("T2-E: Ni-exclusion inverse on the real Veile observables",
                 fontsize=12)
    plt.tight_layout()
    plt.savefig(OUT / "plot_tier2_inverse.png", dpi=150)
    print(f"Saved: {OUT / 'plot_tier2_inverse.png'}")

    with open(OUT / "tier2_inverse_fit.json", "w") as f:
        json.dump({n: float(v) for n, v in zip(PARAMS, sol.x)}, f, indent=1)
    with open(OUT / "tier2_inverse_fit.csv", "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["param", "fit", "sig1_lo", "sig1_hi", "max_dchi2", "verdict"])
        w.writerows(rows)
        w.writerow(["chi2/dof", f"{chi2:.2f}/1", "", "",
                    f"no-ol-supply chi2: {chi2_no_ol:.1f}", ""])
    print(f"Saved: {OUT / 'tier2_inverse_fit.json'}, tier2_inverse_fit.csv")
