"""The spatial identifiability gate — synthetic-profile identifiability gate for the the spatial model spatial model.

the spatial design notes.md Section 5 requires this go/no-go study BEFORE any the spatial model
...
of each would-be the spatial model parameter. Parameters that come back flat are not fittable
from the Veile line-scan data and must be dropped from the the spatial model inverse scope.

Profile-shape parametrization and its mapping to the spatial model physics (the gate fits the
...
for any physics parameter that only enters through it):
    ...
    L_bl      barrier-layer thickness            (the zero-dimensional fit output, known)
...
constant-field limit, which is the designed the spatial model starting point).
...
Fig. 9 scatter), while the fit — like the future the spatial model inverse — recovers one shared
...
Run:  python -m htw_pdm.spatial_identifiability   (~1 min)
"""
...
def main():
    ...
    with open(OUT / "spatial_identifiability.csv", "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["param", "truth", "replicate_mean", "replicate_sd",
                    "rel_sd", "bias", "verdict"])
        w.writerows(rows)
    print(f"Saved: {OUT / 'spatial_identifiability.csv'}")
    ...
    fig.suptitle("the spatial identifiability gate identifiability gate: profile likelihoods on synthetic EDX data",
                 fontsize=13)
    plt.tight_layout()
    out = OUT / "plot_spatial_identifiability.png"
