"""The transient spatial solve — moving-boundary transient defect transport (Landau transform, NSEM).

Solves the Landau-transformed transient problem of src/spatial_physics.py for both
species over tau in [tau0, 1] (t in [24, 480] h), with L(tau) imposed from the
the zero-dimensional fit closed form and the m/bl growth flux jhat(tau) as the the zero-dimensional fit anchor.
Space-time MLP chat(xi, tau) evaluated on the (Nt x Nx) tensor grid; CN in time,
DVR in space; BRDR + TwoPhaseOptimizer.

Regression checks (the the zero-dimensional fit reduction discipline of the spatial design notes Section 3):
  ...
  R2 the zero-dimensional fit flux reduction: m/bl flux at tau = 1 equals the the zero-dimensional fit growth flux
     (jhat = 1) within 1 %.

Run:
    python -m htw_pdm.spatial_transient_trainer
    python -m htw_pdm.spatial_transient_trainer train.n_adam=500   # smoke
"""
...
@hydra.main(config_path=str(CONF), config_name="spatial_config", version_base="1.3")
def main(cfg: DictConfig) -> dict:
    ...
    # log-clustered tau grid on [tau0, 1] (reuse the zero-dimensional fit's builder then rescale).
    ...
    for name in ("OV", "CV"):
        ...
        with torch.no_grad():
            ...
            # R2: the zero-dimensional fit flux reduction at the generation boundary, tau = 1.
        ...
        logger.info(f"[{name}] R2 |m/bl flux - the zero-dimensional fit growth flux| at tau=1: {r2:.3e}")
    ...
    logger.info("the transient spatial solve acceptance: "
