"""The spatial model spatial HTW_PDM physics — defect transport across the barrier layer.

Milestones the steady spatial solve/the transient spatial solve of the spatial design notes.md. Two defect fields on the barrier layer,
...
Nernst-Planck flux with Einstein mobility u = D*gamma (gamma = F/RT):
    ...
    J = -D dc/dx + z*gamma*E*D * c,     E = eps_f (constant, the zero-dimensional fit value)
...
**Structural identifiability note (corroborates the the spatial identifiability gate verdict):** D cancels from
Pe — the steady nondimensional profile shape depends ONLY on z*gamma*E*L, all of
which the zero-dimensional fit already fixes. Steady profile shapes carry NO diffusivity information;
...
Boundary conditions (interfacial-kinetics Robin closure, kappa = k_interface/|v|):
    ...
    OV: J fixed by the zero-dimensional fit growth (J_OV = (chi/2) k3, k3 = (dL/dt)/Omega_bl);
...
Assumed (NOT identified — per the the spatial identifiability gate gate these are forward-only): kappa_a,
kappa_1, r_CV, D_OV, D_CV. the zero-dimensional fit-anchored (identified): eps_f, chi, L(t), k3(t).

Analytic steady solution (verification target for Newton and NSEM):

    chat(xi) = Jhat/s + (chat_bc - Jhat/s) * exp(s*Pe*(xi - xi_bc))

Transient (the transient spatial solve), Landau-transformed to fixed xi in [0,1] with L = L(t) from the
the zero-dimensional fit closed form (tau = t/t_c):
...
# Imported on first `torch.<attr>`, so the classical the spatial model/the nickel-zone closure entry points
# that only need the numpy helpers above can run without PyTorch installed.
torch = lazy_torch()

# the zero-dimensional fit anchors and assumed transport constants (documented above).
EPS_F_V_CM = 1.73e4  # V/cm — the zero-dimensional fit recovered field strength
...
def growth_k3(p1: Parameters, L_nm: float) -> float:
    """the zero-dimensional fit-anchored growth rate constant k3 (mol cm^-2 s^-1) at thickness L."""
...
def species_groups(p1: Parameters, L_nm: float) -> dict[str, SpeciesGroups]:
    """Both species' nondim groups at barrier-layer thickness L (the zero-dimensional fit anchored)."""
    L_cm = L_nm * NM_TO_CM
    N_A = 6.02214076e23
...
class TransientGroups:
    ...
    fixed D). Time-varying pieces enter as tensors over the tau grid:
        ...
        jhat(tau) = k3(L(tau))/k3(L_ref)     (the zero-dimensional fit growth-flux ratio, -> 1)
...
def transient_groups(p1: Parameters, name: str, tau: np.ndarray, t_c_h: float,
    ...
    L, dlnL = zero_d_L_of_tau(p1, tau, t_c_h)
    L_ref = float(L[-1])
...
def transient_residuals(
    ...
    BCs per time node: Jhat = s*jhat(tau) at the generation boundary (the zero-dimensional fit
...
def zero_d_L_of_tau(p1: Parameters, tau: np.ndarray, t_c_h: float):
    """L_bl(tau) and d(ln L)/dtau from the the zero-dimensional fit closed form."""
