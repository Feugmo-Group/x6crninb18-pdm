"""Tier-4 T4-C — a nickel-zone mobility closure of unknown functional form.

Tier 3 tested three closures for the effective nickel mobility -- constant,
...
The differentiable solver. Tier 3 integrates the moving-frame PDE with
...
is verified against the Tier-3 Radau solver at matched parameters (check C0).

Zone widths are extracted with a smooth surrogate

    W = integral_0^ximax sigmoid( (c(xi) - c_thr) / eps ) dxi

which converges to the Tier-3 hard threshold-crossing width as eps -> 0 and is
...
Run:
    ...
    python -m htw_pdm.tier4_ni_closure --smoke
"""
...
N_ENSEMBLE = 12
D_FLOOR, D_CEIL = 1e-2, 1e2  # nm^2/h, the Tier-3 bounds
...
def v_of_t(t_h, p1: Parameters, phi: float) -> float:
    """Interface recession speed, nm/h (Tier-3 definition, unchanged)."""
...
def c0_verification(p1: Parameters) -> dict:
    """The differentiable solver and the smooth width must match Tier 3.

    Compares `wagner_torch` at constant D against the Tier-3 scipy/Radau
...
def fit_once(p1, W_obs, W_sig, t_nodes, seed: int, n_steps: int,
    ...
    net = LogDNet()
    # Disperse the starting mobility across the Tier-3 bounds so the ensemble
...
def main(smoke: bool = False) -> dict:
    ...
    path = os.path.join(out_dir, "tier4_ni_closure.json")
    with open(path, "w") as fh:
        json.dump(res, fh, indent=2, default=float)
    print(f"[T4-C] wrote {path}")
    return res
