...
TIER3_PLAN.md T3-A: the T2-E fit (`tier2_inverse.py`) uses a constant effective
...
to three interchangeable closures for D(t) and frac(t), selected by `closure`:
    ...
    "const"  T2-E baseline: D(t) = D0, frac(t) = 1.            (3 parameters)
    "flux"   M-A, defect-flux-coupled mobility: D(t) decays with the Tier-1
...
Both new closures are physically anchored: M-A reuses the Tier-1 M4 growth
...
assumed. Tier-1/Tier-2 kinetic parameters (A_bl, b3, PBR_eff, C_x, L_ol0) stay
frozen at their M4 values throughout — only the Ni-zone parameters are fit.

This module intentionally does NOT modify `tier2_inverse.py` (the committed,
reported T2-E result) — `closure="const"` here is a from-scratch
reimplementation verified to reproduce T2-E bit-for-bit
(`tests/test_tier3.py::test_const_matches_tier2_inverse`), so the T2-E
...
NI_M = 10.0  # wt% matrix baseline (T2-A/T2-D/T2-E convention)
...
# 72 h Ni peak amplitude from the Fig. 4 scans (T2-E Section 3.3); population SD.
...
def dL_bl_dt(t_h: float, p1: Parameters) -> float:
    """Tier-1 barrier-layer growth flux dL_bl/dt (frozen M4 kinetics), nm/h."""
...
def v_m_of_t(theta: np.ndarray, p1: Parameters):
    """Interface recession speed into the metal, nm/h (Tier-1 kinetics + ol supply)."""
    phi = theta[2]
