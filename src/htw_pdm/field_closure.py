"""The extended solves the field-closure solve — self-consistent Poisson-Nernst-Planck transport in the barrier layer.

The spatial model imposes the PDM's constant-field closure: E(x) = eps_f everywhere, with
eps_f fixed by the the zero-dimensional fit mapping b3 = -alpha3*chi*gamma*eps_f. That closure is
...
state, nondimensional on xi = x/L, with chat_i = c_i/c0_i as in spatial_physics
and the field measured against the the zero-dimensional fit value, Ehat = E/eps_f:
    ...
    Gauge (preserves the the zero-dimensional fit anchor):
...
analytic constant-field solution of spatial_physics.analytic_steady, which is the
exact reference this module is graded against. Lam = O(1) means the constant-field
closure is quantitatively wrong.

Unlike the the spatial model steady problem, the coupled system is *nonlinear* (the product
...
from htw_pdm.spatial_physics import (
    D_CV_CM2_S,
    D_OV_CM2_S,
    EPS_F_V_CM,
    KAPPA_1,
...
class PNPSpecies:
    ...
    Pe: float  # |z| * gamma * eps_f * L  (field reference = the zero-dimensional fit eps_f)
...
def pnp_species(p1: Parameters, L_nm: float, eps_r: float = EPS_R) -> list[PNPSpecies]:
    ...
    Mirrors spatial_physics.species_groups (same anchors, same assumed constants)
    and adds Lam. Keeping the two constructors parallel is deliberate: the field-closure solve must
    reduce to the steady spatial solve exactly when Lam -> 0.
    """
    L_cm = L_nm * NM_TO_CM
...
def pnp_residuals(
    ...
    The spatial model constant-field problem whose analytic solution is known exactly. This
...
def newton_pnp(
    ...
    """
    from htw_pdm.spatial_physics import SpeciesGroups, analytic_steady
...
def field_diagnostics(Ehat: torch.Tensor, w: torch.Tensor) -> dict[str, float]:
    ...
    `max_dev` is the quantity that decides whether the the zero-dimensional fit eps_f is a
