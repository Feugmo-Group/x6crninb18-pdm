"""The extended solves the transient inverse — joint moving-boundary inverse: defect fields and parameters together.

The spatial model solves the Landau-transformed transient problem *forward*, with every
...
have a closed form, so the zero-dimensional fit can build a hard-mode inversion in which the physics
...
That is also exactly the mode that failed in the zero-dimensional fit (failure mode F-1: the data
...
Inverted parameters (per species, in log space so they stay positive):
    ...
    mig    -- the migration prefactor t_c/tau_mig, which is proportional to the
              defect diffusivity D. The the spatial model structural result showed steady
...
Run:
    ...
    python -m htw_pdm.transient_inverse --smoke
"""
...
# The spatial model assumed values. Deliberately off 1.0 so a fit that simply keeps its
...
def cn_forward(tg: TransientGroups, D1x: torch.Tensor, t_grid: torch.Tensor,
    ...
    Step operator, from spatial_physics.transient_residuals:
        dchat/dtau = A_j chat,
...
def main(smoke: bool = False) -> dict:
    ...
    # B0: the classical stepper must reproduce the the spatial model forward physics --
    ...
    path = os.path.join(out_dir, "transient_inverse.json")
    with open(path, "w") as fh:
        json.dump(res, fh, indent=2, default=float)
    print(f"[the transient inverse] wrote {path}")
    return res
