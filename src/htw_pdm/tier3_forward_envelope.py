"""T3-B' -- forward predict-and-compare envelope for the Ni-zone closure.

TIER3_PLAN.md T3-A found BOTH candidate closures (M-A flux-coupled mobility,
...
contingency, Tier 3 re-scopes from "refit T2-E" to forward predict-and-compare
-- the same treatment T2-D gave the unidentifiable in-layer defect gradient.
...
plausible range at the T2-E base kinetics (D_Ni_eff, r_Ni, phi_ol_supply --
frozen at their T2-E fitted values, never re-opened) and reports:
  ...
  2. which part of the n_flux range gives a qualitatively flat/non-monotonic
     ...
     the T2-E const closure, n_flux = 0) -- a descriptive statement, NOT a
...
# never re-opened.  Read from tier2_inverse.py's own output rather than copied:
...
_FIT_JSON = OUT / "tier2_inverse_fit.json"
if not _FIT_JSON.exists():
    raise SystemExit(f"missing {_FIT_JSON}; run src/tier2_inverse.py first")
_fit = json.loads(_FIT_JSON.read_text())
BASE = np.array([_fit["D_Ni_eff_nm2_h"], _fit["r_Ni"], _fit["phi_ol_supply"]])

# Physically plausible n_flux range: the T3-A gate found the flattest
...
def main():
    ...
    print(f"Frozen T2-E base kinetics (D_Ni_eff, r_Ni, phi_ol_supply): {BASE}")
    ...
    print(f"\nconst closure (n_flux=0) reference: W={np.round(W_all[n0_idx],1)}, "
          ...
          f"(matches T2-E: chi2/dof = 25.7/1)")
    ...
    ax1.plot(T_OBS, W_all[n0_idx], "k--", lw=2, label="n_flux = 0 (T2-E const)")
    ...
    ax1.set_title("T3-B': forward n_flux envelope vs real Ni widths\n"
                  "(descriptive only -- T3-A showed n_flux is not fittable)")
    ax1.legend(fontsize=8)
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    fig.colorbar(sm, ax=ax1, label="n_flux")

    ax2.plot(N_FLUX_SCAN, chi2_col, "b-")
    ax2.axhline(25.71, color="grey", ls="--", lw=1, label="T2-E const chi2 (25.7)")
    ...
    plt.tight_layout()
    plt.savefig(OUT / "plot_tier3_forward_envelope.png", dpi=150)
    print(f"\nSaved: {OUT / 'plot_tier3_forward_envelope.png'}")

    with open(OUT / "tier3_forward_envelope.csv", "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["n_flux", "W_72h", "W_168h", "W_480h", "A72", "width_CV",
                    "non_monotonic", "forward_chi2"])
        for r in rows:
            w.writerow(r)
    print(f"Saved: {OUT / 'tier3_forward_envelope.csv'}")
