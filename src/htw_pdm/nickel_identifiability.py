"""The nickel-closure gate -- synthetic identifiability gate for the Ni-exclusion closure family.

the nickel-closure notes.md the nickel-closure gate: before fitting either candidate closure (M-A flux-coupled
...
population-SD weighted) -- the the spatial identifiability gate/M3-trap-avoidance discipline: never fit a
parameter that has not been shown recoverable.

Procedure per closure (D0, r_Ni, phi_ol fixed at the the spatial inverse fitted optimum,
IMPL_REPORT.md the spatial inverse):
  1. pick a truth for the new parameter via a grid search that minimizes the
     ...
     regime" the plan calls for (the constrained spatial inverses own const closure gives monotonically
     growing widths; the whole point of the nickel-zone closure is testing whether a closure
  ...
  2. draw N_REPLICATES synthetic experiments: W_obs, A72_obs from the truth
     ...
     The the spatial identifiability gate finding -- these are already-aggregated summary observables,
     not raw line-scan points, so no separate heterogeneity draw is needed);
  3. refit each replicate with the same multi-start discipline as the spatial inverse;
     report replicate mean/SD -> verdict.

Acceptance (the nickel-closure notes.md the nickel-closure gate): new-parameter replicate rel. SD <= 30% AND
The base parameters (D0, r_Ni, phi_ol) do not degrade beyond 2x their the spatial inverse
profile-likelihood 1-sigma half-widths (SPATIAL_INVERSE_PROFILE_HALFWIDTH below, from the
IMPL_REPORT.md the spatial inverse profile-likelihood scan). If BOTH closures fail -> re-scope
The nickel-zone closure to forward predict-and-compare (a valid, documented outcome, not a bug
to route around).

Run:  python -m htw_pdm.nickel_identifiability
"""
...
# The spatial inverse fitted optimum (D_Ni_eff, r_Ni, phi_ol_supply) -- IMPL_REPORT.md the spatial inverse.
BASE = np.array([3.8, 0.29, 0.57])

# The spatial inverse profile-likelihood 1-sigma HALF-WIDTHS, from a fresh rerun of
# spatial_inverse.py's const-closure fit (chi2/dof = 25.71/1, matching
# IMPL_REPORT.md the spatial inverse exactly) plus a refined profile scan around the
# optimum (the the spatial inverse script's own 20-pt wide-range grid gives degenerate
...
SPATIAL_INVERSE_PROFILE_HALFWIDTH = {
...
NEW_PARAM_GRID = {
    ...
    # at the the spatial inverse base kinetics (checked empirically) -- S_cap <~ 500 (the
...
# Anchored multi-start: (D0, r0, phi0) base combos -- the the spatial inverse fit itself
# (base params are already tightly constrained; the constrained spatial inverses own multi-start only
...
# on the noiseless-recovery check (test_nickel.py) with matching chi2~0 result.
MULTISTART_BASE = [
    (3.85, 0.29, 0.57),   # the spatial inverse optimum
    (1.5, 0.10, 0.30),
    (8.0, 0.60, 0.85),
    (1.5, 0.60, 0.85),
    (8.0, 0.10, 0.30),
...
def pick_truth(closure: str, p1: Parameters):
    """New-parameter value giving the flattest width trajectory at the the spatial inverse
...
def main():
    ...
    halfwidth = SPATIAL_INVERSE_PROFILE_HALFWIDTH
    if halfwidth is None:
        raise RuntimeError(
            "SPATIAL_INVERSE_PROFILE_HALFWIDTH is unset -- patch it from a fresh "
            "spatial_inverse.py profile-likelihood rerun before running the gate "
            "(see the nickel-closure notes.md the nickel-closure gate acceptance criterion)."
        )
    ...
    plt.tight_layout()
    plt.savefig(OUT / "plot_nickel_identifiability.png", dpi=150)
    print(f"\nSaved: {OUT / 'plot_nickel_identifiability.png'}")

    with open(OUT / "nickel_identifiability.csv", "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["closure", "param", "truth", "repl_mean", "repl_sd", "rel_sd"])
        w.writerows(all_rows)
    print(f"Saved: {OUT / 'nickel_identifiability.csv'}")

    overall = "GO" if any(v == "GO" for v in verdicts.values()) else "NO-GO"
    print(f"\nGate verdict per closure: {verdicts} -> overall {overall} "
          f"({'proceed to the nickel forward envelope/C with the passing closure(s)' if overall == 'GO' else 'both closures fail the gate -- re-scope the nickel-zone closure to forward predict-and-compare'})")
    with open(OUT / "nickel_identifiability_verdict.json", "w") as f:
