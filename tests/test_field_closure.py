"""The extended solves regression tests: PNP self-consistency, the differentiable steppers.

Every the extended solves module claims to reduce to an already-verified the spatial model/the nickel-zone closure result
...
Run from the example root:  python -m pytest tests/test_field_closure.py -q
"""
...
# The extended solves is the neural half throughout -- there is no classical subset here to
# preserve, so the whole module stands down when the dependencies are absent.
pytest.importorskip("torch", reason="PyTorch not installed (the extended solves is neural)")
pytest.importorskip(
...
from htw_pdm.field_closure import field_diagnostics, newton_pnp, pnp_species  # noqa: E402
from htw_pdm.nickel_mobility_closure import _tgrid, soft_width, wagner_torch  # noqa: E402
from htw_pdm.nickel_wagner import NI_M, RISE, T_OBS  # noqa: E402
from htw_pdm.nickel_wagner import model as wagner_scipy  # noqa: E402
from htw_pdm.physics import Parameters  # noqa: E402
from htw_pdm.spatial_physics import (  # noqa: E402
    SpeciesGroups,
    analytic_steady,
    transient_groups,
    transient_residuals,
)
from htw_pdm.transient_inverse import (  # noqa: E402
    NT,
    NX,
    T_C_H,
    TAU0,
...
# --------------------------------------------------------------------------
# The field-closure solve: the coupled PNP solver must collapse onto the the spatial model constant field.
# --------------------------------------------------------------------------

def test_pnp_zero_space_charge_matches_analytic():
    """lam_scale = 0 reproduces the the spatial model analytic solution to machine precision."""
    m = _mapper()
...
def test_pnp_gauge_is_enforced():
    """The mean field is pinned to the the zero-dimensional fit value, so eps_f keeps its meaning."""
    m = _mapper()
    species = pnp_species(P1, L_TEST_NM)
...
def test_pnp_lambda_scales_inversely_with_diffusivity():
    ...
    import htw_pdm.field_closure as T
...
# --------------------------------------------------------------------------
# The transient inverse: the classical Crank-Nicolson stepper must satisfy the the spatial model residuals.
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The nickel mobility closure: the differentiable Wagner solver must reproduce the the nickel-zone closure integrator.
# --------------------------------------------------------------------------

def test_wagner_torch_matches_scipy_const_closure():
    """Torch Crank-Nicolson vs the nickel-zone closure scipy/Radau at matched constant D."""
