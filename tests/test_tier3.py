"""T3-A regression tests for the shared Wagner closure engine (tier3_wagner.py).
...
P1 = Parameters()
THETA_CONST = np.array([3.8, 0.29, 0.57])  # T2-E fitted optimum (IMPL_REPORT T2-E)


def test_const_matches_tier2_inverse():
    """closure='const' must reproduce tier2_inverse.py's committed engine exactly."""
    from htw_pdm.tier2_inverse import wagner_solve as t2_wagner_solve
...
def test_noiseless_synthetic_recovery(closure):
    ...
    T3-A). This does NOT assert the recovered point equals the truth: with 4
    ...
    the 'trap' case checked at src/tier3_identifiability.py time). Uniqueness
